...
def test_la_lamina_no_tiene_borde_exterior(tmp_path: Path) -> None:
    ...
    reporte = componer(fotos, tmp_path / "set.jpg")
...
def test_la_lamina_siempre_es_cuadrada(tmp_path: Path, celdas: int) -> None:
    ...
    """
    reporte = componer(_fotos(tmp_path, celdas), tmp_path / "set.jpg")
...
def test_el_bloque_queda_centrado_en_el_cuadro(tmp_path: Path) -> None:
    ...
    """
    reporte = componer(_fotos(tmp_path, 5), tmp_path / "set.jpg")
...
def test_los_huecos_toman_el_color_de_fondo_de_las_fotos(tmp_path: Path) -> None:
    """El color no se recibe como parametro: se lee de la propia foto.

    Pedirlo aparte crearia dos verdades sobre el mismo color, y un set con los
    huecos de otro tono que las celdas es exactamente el defecto que se nota.
    """
    componer(_fotos(tmp_path, 4), tmp_path / "set.jpg")
    with Image.open(tmp_path / "set.jpg") as imagen:
        rgb = imagen.convert("RGB")
        # La esquina de la lamina es separacion pura, nunca celda.
        for punto in ((1, 1), (rgb.width - 2, 1), (1, rgb.height - 2)):
            leido = rgb.getpixel(punto)
            assert all(abs(a - b) <= 2 for a, b in zip(leido, FONDO, strict=True)), (
                f"{punto} no tiene el fondo de las fotos: {leido}"
            )


def test_un_fondo_distinto_cambia_los_huecos(tmp_path: Path) -> None:
    """La otra mitad del test de arriba: que lea el fondo y no una constante."""
    oscuro = (40, 44, 48)
    fotos = [_foto(tmp_path / f"v-{i}.jpg", 200, fondo=oscuro) for i in range(4)]
    componer(fotos, tmp_path / "set.jpg")
    with Image.open(tmp_path / "set.jpg") as imagen:
        leido = imagen.convert("RGB").getpixel((1, 1))
        assert all(abs(a - b) <= 2 for a, b in zip(leido, oscuro, strict=True)), leido
...
def test_cada_celda_trae_su_propia_foto(tmp_path: Path) -> None:
    ...
    """
    reporte = componer(_fotos(tmp_path, 7), tmp_path / "set.jpg")
    with Image.open(tmp_path / "set.jpg") as imagen:
        rgb = imagen.convert("RGB")
...
def test_el_set_entra_en_el_tope_de_dos_megas(tmp_path: Path) -> None:
    ...
    reporte = componer(_fotos(tmp_path, MAX_CELDAS), tmp_path / "set.jpg")
...
def test_recomponer_reemplaza_el_set_anterior(tmp_path: Path) -> None:
    ...
    componer(_fotos(tmp_path, 9), destino)
    primero = destino.stat().st_size
    componer(_fotos(tmp_path, 2), destino)
    ...
    assert componer(_fotos(tmp_path, 2), destino).lado_celda_px > 900
